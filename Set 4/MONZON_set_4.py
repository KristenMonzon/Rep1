...
def dec_to_bin(number):
    ...
    while quotient != 0:
        bin_num = str(int(quotient) % 2)
        quotient = int(number) // 2
        number = quotient
        current_num += bin_num
    final_num = current_num[::-1]
    return final_num
...
def telephone_cipher(message):
    ...
    number_sequence = encoder_dict[message[0]]
    for i in range(1,len(message)):
        element = message[i]
        if encoder_dict[element][0] != number_sequence[-1]:
            number = encoder_dict[element]
...
def telephone_decipher(telephone_string):
    ...
    for i in range(0,len(telephone_string)):
        element = telephone_string[i]

        if current_number == "":
            current_number = element
        elif current_number[-1] == element:
            current_number += element
        elif current_number[-1]== "_":
            current_number = element
        else:
            final_letter = decipher_dict[current_number]
            final_statement += final_letter
            current_number = element
            
    final_statement += decipher_dict[current_number]
