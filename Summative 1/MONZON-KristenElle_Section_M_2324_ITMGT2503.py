...
unnecessary_words = ["<P>", "</P>", "<B>", "</B>", "<I>", "</I>"]

new_data = str(list(map(str.upper,jsondata.values())))
filtered_data_1 = filter(lambda x: x not in unnecessary_words, new_data)
result = ''.join(filtered_data_1)
...
#loop that adds all the words into the dictionary but if len(word) < 3, do not include it
...
#thought process: get F, R, C values in each constraint then get the value that satisfies every constraint which will be equal to optimal_F, optimal_R, and optimal_c and plug it into the function of optimal Z

#getting possible values for F, R, C for each constraint

for i in range(1,30):
    if 1000*i <= 18000//1000 and 500*i <=1200//500 and 50*i <= 1500//50:
        i = F
    if 600*i <= 18000//500 and 750*i <=12000//750 and 45*i <= 1500//45:
        i = R
    if 750*i <= 1800//750 and 300*i <=1200//300 and 75*i <= 1500//75:
        i = C
...
def Collatz(start_number):
    ...
    collatz_dict = dict({})
    collatz_dict["collatz_sequence"] = collatz_sequence
    collatz_dict["collatz_length"] = collatz_length
    collatz_dict["max_collatz"] = max_collatz
    collatz_dict["sequence_sum"] = sequence_sum
...
def Collatz_winner(number_list):
    ...
    for element in number_list:
        ...
        while number != 1:
            ...
            collatz_sequence.append(int(number))
        collatz_length = len(collatz_sequence)
        collatz_length_list.append(int(collatz_length))
        max_collatz = max(collatz_sequence)
        max_collatz_list.append(int(max_collatz))

    if max_collatz_list.index(max(max_collatz_list)) == collatz_length_list.index(max(collatz_length_list)):
        return number_list[max_collatz_list.index(max(max_collatz_list))]
    else
        return "None"
